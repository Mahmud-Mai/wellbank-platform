...
const basicSchema = z.object({
  ...
  description: z
    .string()
    .trim()
    .min(10, "Describe your organization")
    .max(1000),
...
const bankSchema = z.object({
  ...
  settlementFrequency: z.enum(["daily", "weekly"], {
...
export default function NewOrganization() {
  ...
  const bankForm = useForm({
    ...
    defaultValues: {
      ...
      accountNumber: "",
      bvn: "",
      settlementFrequency: undefined as any,
    },
  });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const orgData = {
        name: b.name,
        type: b.type as OrganizationType,
        ...
        ndprConsent: comp.dataPrivacyAgreed,
        termsAccepted: comp.termsAccepted,
        antiFraudDeclaration: comp.antiFraudDeclared,
        slaAccepted: comp.slaAccepted,
        services,
        departments,
        hmosAccepted,
        hasOperatingTheatre,
        ...
        is24Hours,
        acceptsInsurance,
        hasHomeSampleCollection: homeSampleCollection,
        hasDelivery: deliveryAvailable,
        hasColdChain: coldChainCapability,
        handlesControlledDrugs,
        hasGpsTracking: gpsTracking,
        hasColdChainDelivery: coldChainDelivery,
        hasSameDayDelivery: sameDayDelivery,
        hasApiIntegration: apiEnabled,
        productTypes,
        vehicleTypes,
        ambulanceTypes,
      };

      await apiService.organizations.create(orgData);
      ...
      <div className="mx-auto w-full max-w-2xl px-4 py-8">
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Register your healthcare organization on WellBank
            </p>
          </div>
        </div>
        ...
        {step === 1 && (
          <div className="space-y-4">
            ...
            <div>
              ...
              />
              {basicForm.formState.errors.type && (
                <p className="mt-1 text-xs text-destructive">
                  {basicForm.formState.errors.type.message}
                </p>
              )}
            </div>
            <div>
            ...
            <div>
              ...
              <Input
                className="mt-1"
                {...basicForm.register("contactPerson")}
              />
            </div>
            <div>
              <Label>Street Address</Label>
              <Input className="mt-1" {...basicForm.register("street")} />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label>City</Label>
                <Input className="mt-1" {...basicForm.register("city")} />
              </div>
              <div>
                <Label>State</Label>
                <Controller
            ...
            <div className="grid grid-cols-2 gap-3">
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <Select
                      ...
                      disabled={!watchState || !STATE_LGAS[watchState]}
                    >
                      <SelectTrigger className="mt-1">
                        <SelectValue placeholder="Select LGA" />
                      </SelectTrigger>
                      <SelectContent>
                        {(STATE_LGAS[watchState] || []).map((l) => (
              ...
              <div>
                ...
                <Input className="mt-1" {...basicForm.register("cacNumber")} />
              </div>
            </div>
            <div>
              <Label>TIN</Label>
              <Input className="mt-1" {...basicForm.register("tin")} />
            </div>
          </div>
        )}
          ...
          <div className="space-y-4">
            ...
            {(orgType === "hospital" || orgType === "clinic") && (
              <>
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div>
                    <Label>Avg Daily Patients</Label>
                    <Input type="number" className="mt-1" />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
              ...
              <>
                <div className="flex items-center justify-between rounded-lg border border-border p-3">
                  ...
                  <Switch
                    ...
                    onCheckedChange={setHomeSampleCollection}
                  />
                </div>
                <MultiInput
                  label="Tests/Services Offered"
                  values={services}
                  onChange={setServices}
                  placeholder="e.g. Complete Blood Count"
                />
                <div>
                  <Label>Chief Lab Scientist Name</Label>
                  <Input className="mt-1" />
                </div>
                <div>
                  <Label>Chief Lab Scientist MLSN Number</Label>
                  <Input className="mt-1" />
                </div>
              </>
            )}
            ...
            {orgType === "insurance" && (
              <>
                ...
                <div>
                  <Label>Claims Turnaround (Days)</Label>
                  <Input type="number" className="mt-1" />
                </div>
              </>
            )}
            ...
            {orgType === "emergency" && (
              <>
                <div>
                  ...
                  <Input className="mt-1" placeholder="e.g. Lagos Mainland" />
                </div>
                <div>
                  ...
                  <div className="mt-2 flex gap-4">
                    {AMBULANCE_TYPES.map((t) => (
                      <div key={t} className="flex items-center gap-2">
                        <Checkbox
                          checked={ambulanceTypes.includes(t)}
                          onCheckedChange={() =>
                            setAmbulanceTypes((prev) =>
                              prev.includes(t)
                                ? prev.filter((x) => x !== t)
                                : [...prev, t],
                            )
                ...
                <div>
                  ...
                  <Input className="mt-1" placeholder="e.g. 15 minutes" />
                </div>
              </>
            )}
        ...
        {step === 3 && (
          <div className="space-y-4">
            ...
            <FileUpload
              ...
              value={docs.cacCert || null}
              onChange={(f) => setDoc("cacCert", f)}
              required
            />
            <FileUpload
              label="Tax Clearance"
              value={docs.taxClearance || null}
              onChange={(f) => setDoc("taxClearance", f)}
              required
            />
            <FileUpload
              label="Logo"
              value={docs.logo || null}
              onChange={(f) => setDoc("logo", f)}
              accept=".jpg,.jpeg,.png"
            ...
            {(orgType === "hospital" || orgType === "clinic") && (
              <>
                <FileUpload
                  label="State Health License"
                  value={docs.healthLicense || null}
                  onChange={(f) => setDoc("healthLicense", f)}
                  required
                />
                <FileUpload
                  label="Operating License"
                  value={docs.operatingLicense || null}
                  onChange={(f) => setDoc("operatingLicense", f)}
                  required
                />
                <FileUpload
                  label="Environmental Permit"
                  value={docs.envPermit || null}
                  onChange={(f) => setDoc("envPermit", f)}
                />
                <FileUpload
                  label="Medical Director License"
                  value={docs.mdLicense || null}
                  onChange={(f) => setDoc("mdLicense", f)}
                  required
                />
                <div>
                ...
                <div>
                  <Label>Medical Director MDCN</Label>
                  <Input className="mt-1" />
                </div>
              </>
            )}
            ...
            {orgType === "laboratory" && (
              <>
                <FileUpload
                  label="MLSCN License"
                  value={docs.mlscn || null}
                  onChange={(f) => setDoc("mlscn", f)}
                  required
                  description="Medical Laboratory Science Council"
                />
                <FileUpload
                  label="Practice License"
                  value={docs.practiceLicense || null}
                  onChange={(f) => setDoc("practiceLicense", f)}
                  required
                />
                <FileUpload
                  label="Environmental Permit"
            ...
            {orgType === "pharmacy" && (
              <>
                <FileUpload
                  label="PCN License"
                  value={docs.pcn || null}
                  onChange={(f) => setDoc("pcn", f)}
                  required
                  description="Pharmacists Council of Nigeria"
                />
                <FileUpload
                  label="Superintendent License"
                  value={docs.superLicense || null}
                  onChange={(f) => setDoc("superLicense", f)}
                  required
                />
                <FileUpload
                  label="Premises License"
                  value={docs.premises || null}
                  onChange={(f) => setDoc("premises", f)}
                  required
                />
              </>
            )}
            ...
            {orgType === "insurance" && (
              <>
                <FileUpload
                  ...
                  value={docs.naicom || null}
                  onChange={(f) => setDoc("naicom", f)}
                  required
                />
                <FileUpload
                  label="HMO License"
                  value={docs.hmo || null}
                  onChange={(f) => setDoc("hmo", f)}
            ...
            {orgType === "emergency" && (
              <>
                <FileUpload
                  label="State Ministry Approval"
                  value={docs.stateApproval || null}
                  onChange={(f) => setDoc("stateApproval", f)}
                  required
                />
                <FileUpload
                  label="Ambulance Service License"
                  value={docs.ambulanceLicense || null}
                  onChange={(f) => setDoc("ambulanceLicense", f)}
                  required
                />
                <FileUpload
                  label="Vehicle Registrations"
                  value={docs.vehicleReg || null}
                  onChange={(f) => setDoc("vehicleReg", f)}
                  required
                />
                <FileUpload
                  label="Road Worthiness"
                  value={docs.roadWorthiness || null}
                  onChange={(f) => setDoc("roadWorthiness", f)}
                />
                <FileUpload
                  label="Paramedic Certification"
                  value={docs.paramedic || null}
                  onChange={(f) => setDoc("paramedic", f)}
                />
              </>
            )}
            ...
            {orgType === "logistics" && (
              <>
                <FileUpload
                  ...
                  value={docs.companyInsurance || null}
                  onChange={(f) => setDoc("companyInsurance", f)}
                  required
                />
                <FileUpload
                  label="Driver License (sample)"
                  value={docs.driverLicense || null}
                  onChange={(f) => setDoc("driverLicense", f)}
                />
                <FileUpload
        ...
        {step === 4 && (
          <div className="space-y-4">
            ...
            <div>
              <Label>BVN (Corporate Signatory)</Label>
              <Input
                className="mt-1"
                maxLength={11}
              ...
              {bankForm.formState.errors.bvn && (
                <p className="mt-1 text-xs text-destructive">
                  {bankForm.formState.errors.bvn.message}
                </p>
              )}
            </div>
            <div>
        ...
        {step === 5 && (
          <div className="space-y-4">
            ...
            {[
              {
                name: "dataPrivacyAgreed" as const,
                label:
                  "I agree to the Data Privacy Agreement (NDPR Compliance)",
              },
              {
              ...
              {
                name: "antiFraudDeclared" as const,
                label: "I declare the Anti-Fraud Declaration",
              },
              {
                name: "slaAccepted" as const,
                label: "I accept the Service Level Agreement (SLA)",
              },
            ].map(({ name, label }) => (
              <Controller
                key={name}
            ...
            {complianceForm.formState.errors.dataPrivacyAgreed && (
          ...
          {step < 5 ? (
            ...
            >
              {isSubmitting ? (
                ...
                <>
                  Create Organization <Check className="ml-1 h-4 w-4" />
                </>
              )}
            </Button>
          )}
