...
const WellPointsPage = () => {
  ...
  const { data: balanceData, isLoading: balanceLoading } = useQuery({
    ...
    queryFn: () => apiService.wellpoints.getBalance(),
  });

  const { data: txData, isLoading: txLoading } = useQuery({
    queryKey: ["wellpoints-transactions", txPage],
    queryFn: () =>
      apiService.wellpoints.getTransactions({ page: txPage, perPage: 5 }),
  });

  const { data: rulesData } = useQuery({
    queryKey: ["wellpoints-rules"],
    queryFn: () => apiService.wellpoints.getEarningRules(),
  });

  const { data: marketData } = useQuery({
    queryKey: ["wellpoints-marketplace"],
    queryFn: () => apiService.wellpoints.getMarketplace(),
  });
  ...
  const redeemMutation = useMutation({
    mutationFn: (rewardId: string) => apiService.wellpoints.redeem(rewardId),
    onSuccess: (res) => {
  ...
  const nextMilestone = milestones.find(
    (m) => tierOrder.indexOf(m.tier) > currentTierIdx,
  );
          ...
          <Card className="overflow-hidden border-0 bg-gradient-to-br from-accent/20 via-accent/10 to-background shadow-glow">
            <CardContent className="p-6">
              ...
              <div className="mt-4 flex items-center gap-2">
                <Badge
                  className={`capitalize ${tierColors[balance.tier] ?? ""}`}
                >
                ...
                <div className="mt-4">
                  <div className="flex items-center justify-between text-xs text-muted-foreground">
                    ...
                    <span>
                      ...
                      {nextMilestone.points.toLocaleString()}
                    </span>
                  </div>
              ...
              {balance.expiringPoints > 0 && (
                <div className="mt-3 flex items-center gap-2 rounded-lg bg-destructive/10 px-3 py-2">
                  ...
                  <span className="text-xs text-destructive">
                    {balance.expiringPoints} points expiring{" "}
                    {formatDate(balance.expiryDate)}
                  </span>
                </div>
              )}
            </CardContent>
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-3">
          {rewards.map((rw) => (
            <Card
              key={rw.id}
              className="border-border hover:border-accent/30 transition-all"
            >
