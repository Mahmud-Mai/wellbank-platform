...
export default function DoctorOnboarding() {
  ...
  const [practicingLicenseExpiry, setPracticingLicenseExpiry] = useState<
    Date | undefined
  >();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const doctorData = {
        ...
        subSpecialty: pr.subSpecialty,
        yearsExperience: pr.yearsExperience,
        consultationTypes,
        hospitalAffiliations,
        consultationFee: pr.consultationFee,
        bio: pr.bio,
        mdcnLicenseNumber: c.mdcnLicenseNumber,
        practicingLicenseExpiry: practicingLicenseExpiry
          ? format(practicingLicenseExpiry, "yyyy-MM-dd")
          : "",
        bankName: b.bankName,
        accountName: b.accountName,
        accountNumber: b.accountNumber,
        bvn: b.bvn,
        governmentIdType: id.governmentIdType,
      };

      await apiService.doctors.completeProfile(doctorData);
    ...
    <div className="flex min-h-screen bg-background">
      <div className="mx-auto w-full max-w-2xl px-4 py-8">
        <h1 className="mb-1 text-2xl font-bold text-foreground">
          {isAddingRole ? "Doctor Profile Setup" : "Doctor Onboarding"}
        </h1>
        <p className="mb-6 text-sm text-muted-foreground">
          {isAddingRole
            ? "Set up your doctor profile to start accepting patients"
        ...
        {step === 1 && (
          <div className="space-y-4">
                ...
                <Popover>
                  ...
                  <PopoverContent className="w-auto p-0" align="start">
                    <Calendar
                      ...
                      disabled={(d: Date) =>
                        d > new Date() || d < new Date("1920-01-01")
                      }
                      initialFocus
            ...
            <div>
              <Label>Street</Label>
              <Input className="mt-1" {...personalForm.register("street")} />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label>City</Label>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                />
                {profForm.formState.errors.specialty && (
                  <p className="mt-1 text-xs text-destructive">
                    {profForm.formState.errors.specialty.message}
                  </p>
                )}
              </div>
              <div>
                ...
                <Input
                  className="mt-1"
                  {...profForm.register("subSpecialty")}
                />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input
                  type="number"
                  className="mt-1"
                  {...profForm.register("yearsExperience")}
                />
              </div>
              <div>
                ...
                <Input
                  type="number"
                  className="mt-1"
                  {...profForm.register("consultationFee")}
                />
              </div>
            </div>
            <div>
              ...
              <div className="mt-2 flex gap-4">
                {["Virtual", "Physical"].map((t) => (
                  <div key={t} className="flex items-center gap-2">
                    <Checkbox
                      checked={consultationTypes.includes(t)}
                      onCheckedChange={() => toggleConsultationType(t)}
                      id={`ct-${t}`}
                    />
                    <Label htmlFor={`ct-${t}`} className="cursor-pointer">
                      {t}
                    </Label>
                  </div>
                ))}
            ...
            <MultiInput
              ...
              onChange={setHospitalAffiliations}
        ...
        {step === 3 && (
          <div className="space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              Upload your professional documents (PDF/JPG)
            </p>
            <FileUpload
              label="MBBS Certificate"
              value={mbbsCert}
              onChange={setMbbsCert}
              required
            />
            <FileUpload
              label="MDCN License"
              value={mdcnLicense}
              onChange={setMdcnLicense}
              required
            ...
            <div>
              ...
              <Input
                className="mt-1"
                placeholder="e.g. MDCN-001234"
                {...certForm.register("mdcnLicenseNumber")}
              />
              {certForm.formState.errors.mdcnLicenseNumber && (
                <p className="mt-1 text-xs text-destructive">
                  {certForm.formState.errors.mdcnLicenseNumber.message}
                </p>
              )}
            </div>
            <FileUpload
              label="Annual Practicing License"
              value={practicingLicense}
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    ...
                    className={cn(
                      "mt-1 w-full justify-start text-left font-normal",
                      !practicingLicenseExpiry && "text-muted-foreground",
                    )}
                  >
                    <CalendarIcon className="mr-2 h-4 w-4" />
                    {practicingLicenseExpiry
                      ? format(practicingLicenseExpiry, "PPP")
                      : "Select date"}
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-auto p-0" align="start">
                  <Calendar
                    mode="single"
                    selected={practicingLicenseExpiry}
                    onSelect={setPracticingLicenseExpiry}
                    initialFocus
                    className={cn("p-3 pointer-events-auto")}
                  />
                </PopoverContent>
            ...
            <FileUpload
              label="NYSC Certificate / Exemption"
              value={nyscCert}
              onChange={setNyscCert}
              required
            />
            <FileUpload
            ...
            <FileUpload
              ...
              value={otherCerts}
              onChange={setOtherCerts}
              description="Optional additional certificates"
            />
          </div>
        )}
        ...
        {step === 4 && (
          <div className="space-y-4">
            ...
            <div>
              ...
              />
              {idForm.formState.errors.governmentIdType && (
                <p className="mt-1 text-xs text-destructive">
                  {idForm.formState.errors.governmentIdType.message}
                </p>
              )}
            </div>
            <FileUpload
        ...
        {step === 5 && (
          <div className="space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              For consultation fee settlement
            </p>
            <div>
          ...
          {step < 5 ? (
            ...
            >
              {isSubmitting ? (
                ...
                <>
                  Submit Profile <Check className="ml-1 h-4 w-4" />
                </>
              )}
            </Button>
          )}
