...
## 11. Onboarding Module (Patient/Doctor/Organization)

### 11.1 Complete Patient Profile

**Endpoint:** `POST /patients/complete-profile`

**Description:** Complete patient onboarding with all required fields
...
```json
{
  ...
  "idType": "string (enum: NIN, BVN, VOTER_CARD, DRIVERS_LICENSE, PASSPORT)",
  "idNumber": "string",
...
### 11.2 Complete Doctor Profile

**Endpoint:** `POST /doctors/complete-profile`

**Description:** Complete doctor onboarding with credentials and documents
...
```json
{
  ...
  "mdcnExpiryDate": "ISO 8601 date",
  "practicingLicenseNumber": "string",
  "practicingLicenseExpiry": "ISO 8601 date",
  "qualifications": [
    {
      "degree": "string",
      "institution": "string",
...
### 11.3 Create Organization (Hospital/Lab/Pharmacy/Insurance/Emergency/Logistics)

**Endpoint:** `POST /organizations`

**Description:** Create a new organization (requires provider_admin role)
...
```json
{
  "name": "string (required)",
  "tradingName": "string",
  "type": "string (enum: hospital, laboratory, pharmacy, clinic, insurance, emergency, logistics)",
  "description": "string",
  "email": "string",
  "phoneNumber": "string",
...
```json
{
  "ownershipType": "string (enum: private, public, faith_based, ngo)",
  "yearEstablished": "number",
  "facilityType": "string (enum: primary_care_clinic, secondary_hospital, tertiary_hospital, specialist_hospital)",
  "bedCapacity": "number",
...
```json
{
  ...
  "ambulanceTypes": ["basic_life_support", "advanced_life_support"],
...
**Banking & Compliance (All Organizations):**
```json
{
  "bankName": "string",
  "bankAccountName": "string",
  "bankAccountNumber": "string",
  "bankBvn": "string",
  "settlementFrequency": "string (enum: daily, weekly)",
  "ndprConsent": "boolean",
...
**Description:** Upload a document (license, certificate, ID, etc.)
...
```json
{
  "documentType": "string (enum: mbbs_certificate, mdcn_license, pcn_license, mlscn_license, cac_certificate, national_id, etc.)",
  "documentNumber": "string",
  "issueDate": "ISO 8601 date",
  "expiryDate": "ISO 8601 date",
  "fileUrl": "string (URL from file upload)"
}
```
...
```json
{
  ...
  "data": {
    "id": "uuid",
    "documentType": "mdcn_license",
    "documentNumber": "MDCN-123456",
...
## 13. Bank Accounts Module

### 13.1 Add Bank Account

**Endpoint:** `POST /bank-accounts`

**Description:** Add a bank account for settlements
...
**Path Parameters:**
- `id` (uuid, required): Bank account ID
...
## 14. Multi-Role Management Module

### 14.1 Add New Role

**Endpoint:** `POST /auth/role/add`

**Description:** Add a new role to existing user (e.g., patient becomes also doctor)
...
```json
{
  "role": "string (enum: patient, doctor, provider_admin)"
}
```
...
### 14.2 Switch Active Role

**Endpoint:** `POST /auth/role/switch`

**Description:** Switch the active role for the user
...
```json
{
  "activeRole": "string (enum: patient, doctor, provider_admin)"
}
```
